---
name: upgrade
description: Smart migration from v1.x to v2.0. Detects current layout, renames panel- prefixed directories, restructures into papers/publications, installs Makefiles, migrates reviewer profiles to .roles/roles, updates panel.json. Safe — shows plan before touching anything.
user-invocable: true
allowed-tools:
  - Read
  - Write
...
- Reviewer profiles in `.roles/panel-reviewer/` (not `context/`)
...
const plan = {
    ...
    configUpdates: [],
    makefileUpdates: [],
    profileMoves: [],  // context/panel/reviewers/profiles/custom → .roles/panel-reviewer/
...
function detectCustomReviewerProfiles() {
    ...
    const standard = new Set(
        glob(`${pluginRoot}/.roles/panel-reviewer/R-*.md`)
            .map(f => basename(f))
    );
...
- Custom profiles (non-R-N) → move to `.roles/panel-reviewer/`
...
Reviewer profiles:
  ...
  context/panel/reviewers/profiles/my-custom.md        →  .roles/panel-reviewer/my-custom.md
...
# Move custom profiles to .roles/roles
for profile in customProfiles:
    mkdir -p .roles/panel-reviewer
    git mv ${profile} .roles/panel-reviewer/${basename(profile)}
...
await gitCommitIfEnabled(
    ...
    `Reviewer profiles: ${customCount} custom → .roles/roles, standard removed\n` +
...
✓ Reviewer profiles migrated
    ...
    1 custom profile → .roles/panel-reviewer/my-custom.md
