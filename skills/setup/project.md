...
The profile loader reads directly from `${pluginRoot}/.roles/panel-reviewer/` — no file copying needed. Users who want to add custom reviewer profiles create them in their project's `.roles/panel-reviewer/` directory.

```javascript
// Verify plugin reviewer profiles are accessible via pluginRoot
const pluginRolesPath = `${pluginRoot}/.roles/panel-reviewer`;
...
if (!indexExists) {
    ...
    msg(`Extensions: add custom profiles to .roles/panel-reviewer/`, 'item');
}
```
...
```
Reviewer Profiles:
  ✓ ${pluginRoot}/.roles/panel-reviewer/_index.yaml accessible
  ✓ 51 R-N.md profiles accessible (OLE format, Spec 93)
  — Extensions: .roles/panel-reviewer/ (local, optional)
```
